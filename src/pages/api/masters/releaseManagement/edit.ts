...
const addPrasadCategory = async (data: release) => {
  const { id, version_number, build_number, version_date, device_type_id, is_backward_compatible, popup_count } = data
  try {
    await prisma.releaseManagement.update({
      where: { id },
      data: {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (!req?.body) {
    ...
    try {
      ...
      res.status(201).json({ message: 'Release updated successfully' })
    } catch (e) {
